...
async function snapshot(workspaceId: string) {
  ...
  const [workspace, product, agents, tasks, approvals, runs, opportunities, memories, connections, metrics, campaigns, campaignAssets, growthSnapshots, runtimeSettings, runtimeCycle, runtimeUsage, goals, intelligence, brainFacts, brainPacks, brainDecisions, brainExperiments, brainLessons] = await Promise.all([
    ...
    db.prepare("SELECT id, title, rationale, score, confidence, status, created_at AS createdAt FROM decision_journal WHERE workspace_id = ? ORDER BY id DESC LIMIT 8").bind(workspaceId).all(),
