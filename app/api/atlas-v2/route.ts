...
import { ensureDefaultCompanyRoutines, inferRoutine, runCompanyRoutine } from "../../../lib/company-routines";
import { ensureActionTrustPolicies, normalizeTrustLevel } from "../../../lib/company-trust";
...
async function snapshot(workspaceId: string) {
  const db = env.DB;
  const [workspace, product, agents, tasks, approvals, runs, opportunities, memories, connections, metrics, campaigns, campaignAssets, growthSnapshots, runtimeSettings, runtimeCycle, runtimeUsage, goals, intelligence, brainFacts, brainPacks, brainDecisions, brainExperiments, brainLessons, strategyPerformance, companyFunctions, runtimeHeartbeat, routines, routineRuns, trustPolicies, feedback, behaviorRules] = await Promise.all([
    ...
    db.prepare("SELECT snapshot_date AS snapshotDate, goal_id AS goalId, metric_name AS metricName, metric_value AS metricValue, previous_value AS previousValue, metric_delta AS metricDelta, health_score AS healthScore, evidence_json AS evidenceJson, summary, updated_at AS updatedAt FROM company_intelligence_snapshots WHERE workspace_id = ? ORDER BY snapshot_date DESC LIMIT 1").bind(workspaceId).first(),
    db.prepare("SELECT fact_type AS factType, subject, value_json AS valueJson, source, confidence, status, last_verified_at AS lastVerifiedAt FROM company_facts WHERE workspace_id = ? AND status = 'active' ORDER BY confidence DESC, last_verified_at DESC LIMIT 12").bind(workspaceId).all(),
    db.prepare("SELECT pack_key AS packKey, name, description, industry, stage, channel, version FROM knowledge_packs WHERE status = 'active' AND (scope = 'atlas' OR workspace_id = ?) ORDER BY scope, name LIMIT 12").bind(workspaceId).all(),
    db.prepare("SELECT id, title, rationale, alternatives_json AS alternativesJson, context_json AS contextJson, score, confidence, status, created_at AS createdAt FROM decision_journal WHERE workspace_id = ? ORDER BY id DESC LIMIT 8").bind(workspaceId).all(),
    db.prepare("SELECT e.id, e.name, e.hypothesis, e.strategy_key AS strategyKey, e.channel, e.primary_metric AS primaryMetric, e.baseline_value AS baselineValue, e.target_value AS targetValue, e.status, e.evaluation_starts_at AS evaluationStartsAt, e.evaluation_ends_at AS evaluationEndsAt, r.metric_value AS metricValue, r.outcome, r.confidence AS resultConfidence, r.summary AS resultSummary, r.measured_at AS measuredAt FROM growth_experiments e LEFT JOIN experiment_results r ON r.experiment_id = e.id AND r.workspace_id = e.workspace_id WHERE e.workspace_id = ? ORDER BY e.id DESC LIMIT 12").bind(workspaceId).all(),
    db.prepare("SELECT strategy_key AS strategyKey, lesson_type AS lessonType, statement, confidence, status FROM strategy_lessons WHERE workspace_id = ? AND status = 'active' ORDER BY confidence DESC, updated_at DESC LIMIT 8").bind(workspaceId).all(),
    db.prepare("SELECT strategy_key AS strategyKey, experiments_count AS experimentsCount, success_count AS successCount, failure_count AS failureCount, consecutive_failures AS consecutiveFailures, weight, suppressed_until AS suppressedUntil, last_outcome AS lastOutcome, last_evaluated_at AS lastEvaluatedAt FROM strategy_performance WHERE workspace_id = ? ORDER BY weight DESC, updated_at DESC LIMIT 12").bind(workspaceId).all(),
    db.prepare("SELECT function_key AS functionKey, status, confidence, summary, next_action AS nextAction, evidence_json AS evidenceJson, metric_name AS metricName, metric_value AS metricValue, observed_at AS observedAt FROM company_function_snapshots WHERE workspace_id = ? ORDER BY CASE function_key WHEN 'growth' THEN 1 WHEN 'product' THEN 2 WHEN 'sales' THEN 3 ELSE 4 END").bind(workspaceId).all(),
    db.prepare("SELECT source, status, started_at AS startedAt, completed_at AS completedAt, next_expected_at AS nextExpectedAt, CASE WHEN next_expected_at IS NOT NULL AND datetime(next_expected_at, '+30 minutes') < datetime('now') THEN 1 ELSE 0 END AS stale, details_json AS detailsJson, updated_at AS updatedAt FROM runtime_heartbeats WHERE workspace_id = ?").bind(workspaceId).first(),
    db.prepare("SELECT id, routine_key AS routineKey, name, instruction, playbook_key AS playbookKey, trigger_type AS triggerType, cadence_minutes AS cadenceMinutes, event_type AS eventType, trust_level AS trustLevel, status, next_run_at AS nextRunAt, last_run_at AS lastRunAt, last_status AS lastStatus, last_summary AS lastSummary FROM company_routines WHERE workspace_id = ? ORDER BY CASE status WHEN 'active' THEN 1 ELSE 2 END, id").bind(workspaceId).all(),
    db.prepare("SELECT id, routine_id AS routineId, trigger_type AS triggerType, status, summary, started_at AS startedAt, completed_at AS completedAt FROM company_routine_runs WHERE workspace_id = ? ORDER BY id DESC LIMIT 12").bind(workspaceId).all(),
    db.prepare("SELECT action_type AS actionType, trust_level AS trustLevel, max_risk_level AS maxRiskLevel, reason, updated_at AS updatedAt FROM action_trust_policies WHERE workspace_id = ? ORDER BY id").bind(workspaceId).all(),
    db.prepare("SELECT id, target_type AS targetType, target_id AS targetId, feedback_type AS feedbackType, reason, created_by AS createdBy, created_at AS createdAt FROM founder_feedback WHERE workspace_id = ? ORDER BY id DESC LIMIT 12").bind(workspaceId).all(),
    db.prepare("SELECT id, category, rule_text AS ruleText, confidence, status, updated_at AS updatedAt FROM company_behavior_rules WHERE workspace_id = ? ORDER BY CASE status WHEN 'active' THEN 1 ELSE 2 END, id DESC LIMIT 20").bind(workspaceId).all(),
  ]);
  const pending = approvals.results.filter((item) => item.status === "pending");
  const activeTasks = tasks.results.filter((item) => ["queued", "running", "waiting_approval", "approved"].includes(String(item.status))).slice(0, 3);
  const completedTasks = tasks.results.filter((item) => item.status === "completed").slice(0, 5);
  const handoff = {
    status: pending.length ? "waiting_decision" : runtimeCycle?.status === "running" ? "working" : runtimeHeartbeat ? "observing" : "ready",
    statusDetail: pending.length ? `${pending.length} Founder decision(s) are waiting.` : runtimeCycle?.summary ?? "Atlas is observing the company and preparing the next best action.",
    completed: completedTasks.map((item) => ({ id: item.id, title: item.title, completedAt: item.createdAt })),
    discoveries: opportunities.results.slice(0, 3).map((item) => ({ id: item.id, title: item.title, confidence: item.confidence, summary: item.summary })),
    plan: activeTasks.map((item) => ({ id: item.id, title: item.title, status: item.status, expectedOutcome: item.expectedOutcome })),
    founderDecision: pending[0] ? { id: pending[0].id, title: pending[0].title, reason: pending[0].reason, riskLevel: pending[0].riskLevel } : null,
  };
  return { workspace: { id: workspaceId, autonomyEnabled: (workspace as { autonomyEnabled?: number } | null)?.autonomyEnabled !== 0, autonomyUpdatedAt: (workspace as { autonomyUpdatedAt?: string | null } | null)?.autonomyUpdatedAt ?? null }, product: product ? { ...product, analysis: parse((product as { analysisJson?: string }).analysisJson, null) } : null, agents: agents.results.map((i) => ({ ...i, tools: parse(i.tools, []) })), tasks: tasks.results.map((i) => ({ ...i, requiresApproval: Boolean(i.requiresApproval), evidence: parse(i.evidence, []) })), approvals: approvals.results, runs: runs.results.map((i) => ({ ...i, tools: parse(i.tools, []) })), opportunities: opportunities.results, memories: memories.results, connections: connections.results, campaigns: campaigns.results, campaignAssets: campaignAssets.results, growthSnapshots: growthSnapshots.results.map((item) => ({ ...item, reflection: parse(item.reflectionJson, null) })), handoff, companyRuntime: { settings: runtimeSettings ?? null, latestCycle: runtimeCycle ?? null, usage: runtimeUsage ?? { cyclesCount: 0, actionsCount: 0, externalActionsCount: 0, estimatedCostCents: 0 }, goals: goals.results, heartbeat: runtimeHeartbeat ? { ...runtimeHeartbeat, stale: Boolean((runtimeHeartbeat as { stale?: number }).stale), details: parse((runtimeHeartbeat as { detailsJson?: string }).detailsJson, {}) } : null }, companyIntelligence: intelligence ? { ...intelligence, evidence: parse((intelligence as { evidenceJson?: string }).evidenceJson, []) } : null, companyFunctions: companyFunctions.results.map((item) => ({ ...item, evidence: parse((item as { evidenceJson?: string }).evidenceJson, []) })), companyBrain: { facts: brainFacts.results.map((item) => ({ ...item, value: parse((item as { valueJson?: string }).valueJson, null) })), knowledgePacks: brainPacks.results, decisions: brainDecisions.results.map((item) => ({ ...item, alternatives: parse((item as { alternativesJson?: string }).alternativesJson, []), context: parse((item as { contextJson?: string }).contextJson, {}) })), experiments: brainExperiments.results, lessons: brainLessons.results, strategyPerformance: strategyPerformance.results }, routines: { items: routines.results, runs: routineRuns.results }, trust: { policies: trustPolicies.results }, founderFeedback: feedback.results, behaviorRules: behaviorRules.results, metrics: { ...(metrics ?? { visits: 0, signups: 0, paid: 0, conversion: 0, completedTasks: 0 }), yesterdayCompleted: (metrics as { completedTasks?: number } | null)?.completedTasks ?? 0 } };
}
...
async function workspacePayload(workspaceId: string, userId: string) {
  ...
  await ensureDefaultCompanyRoutines(env.DB, workspaceId);
  await ensureActionTrustPolicies(env.DB, workspaceId);
...
export async function POST(request: Request) {
  try {
    const { user, workspaceId } = await getWorkspace(request);
    const body = await request.json() as { action?: string; id?: number; enabled?: boolean; opportunityId?: number; objective?: string; channels?: string[]; locale?: "zh" | "en"; title?: string; content?: string; cta?: string; publishedUrl?: string; scheduledFor?: string; reason?: string; saveAsRule?: boolean; factType?: string; subject?: string; value?: string; instruction?: string; actionType?: string; trustLevel?: string; maxRiskLevel?: number; runtime?: { mode?: "manual" | "copilot" | "autonomous" | "paused"; tickIntervalMinutes?: number; dailyActionLimit?: number; dailyLlmBudgetCents?: number; dailyExternalActionLimit?: number; quietHoursStart?: string | null; quietHoursEnd?: string | null; autoExecuteRiskLevel?: number }; metrics?: { impressions?: number; clicks?: number; conversions?: number }; productName?: string; product?: { name?: string; url?: string; description?: string; growthGoal?: string; locale?: "zh" | "en" } };
    const db = env.DB;
    ...
    if (body.action === "update_company_fact") {
      const factType = body.factType?.trim().slice(0, 48);
      const subject = body.subject?.trim().slice(0, 72);
      const value = body.value?.trim().slice(0, 3000);
      if (!factType || !subject || !value) return Response.json({ error: "Fact type, subject, and value are required." }, { status: 400 });
      const existing = await db.prepare("SELECT id, value_json AS valueJson FROM company_facts WHERE workspace_id = ? AND fact_type = ? AND subject = ?").bind(workspaceId, factType, subject).first<{ id: number; valueJson: string }>();
      await db.prepare("INSERT INTO company_facts (workspace_id, fact_type, subject, value_json, source, evidence_json, confidence, status, last_verified_at, updated_at) VALUES (?, ?, ?, ?, 'Founder correction', '[]', 100, 'active', ?, ?) ON CONFLICT(workspace_id, fact_type, subject) DO UPDATE SET value_json = excluded.value_json, source = excluded.source, confidence = 100, status = 'active', last_verified_at = excluded.last_verified_at, updated_at = excluded.updated_at").bind(workspaceId, factType, subject, JSON.stringify(value), nowText(), nowText()).run();
      await db.prepare("INSERT INTO founder_feedback (workspace_id, target_type, target_id, feedback_type, reason, original_json, corrected_json, created_by) VALUES (?, 'company_fact', ?, 'corrected', ?, ?, ?, ?)").bind(workspaceId, `${factType}:${subject}`, body.reason?.trim().slice(0, 1000) || "Founder updated the company profile.", existing?.valueJson ?? "{}", JSON.stringify(value), user.email).run();
      return Response.json(await workspacePayload(workspaceId, user.id));
    }
    if (body.action === "create_routine") {
      if (!body.instruction?.trim()) return Response.json({ error: "Describe what Atlas should do." }, { status: 400 });
      const routine = inferRoutine(body.instruction);
      const key = `${routine.routineKey}-${Date.now().toString(36)}`;
      await db.prepare("INSERT INTO company_routines (workspace_id, routine_key, name, instruction, playbook_key, trigger_type, cadence_minutes, event_type, trust_level, status, next_run_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)").bind(workspaceId, key, routine.name, routine.instruction, routine.playbookKey, routine.triggerType, routine.cadenceMinutes, routine.eventType, routine.trustLevel, routine.triggerType === "schedule" ? nowText() : null, nowText(), nowText()).run();
      return Response.json(await workspacePayload(workspaceId, user.id));
    }
    if (body.action === "toggle_routine") {
      if (!body.id) return Response.json({ error: "Routine is required." }, { status: 400 });
      await db.prepare("UPDATE company_routines SET status = CASE WHEN status = 'active' THEN 'paused' ELSE 'active' END, next_run_at = CASE WHEN status = 'active' THEN next_run_at ELSE CURRENT_TIMESTAMP END, updated_at = ? WHERE id = ? AND workspace_id = ?").bind(nowText(), body.id, workspaceId).run();
      return Response.json(await workspacePayload(workspaceId, user.id));
    }
    if (body.action === "run_routine") {
      if (!body.id) return Response.json({ error: "Routine is required." }, { status: 400 });
      const routine = await db.prepare("SELECT id, workspace_id AS workspaceId, routine_key AS routineKey, name, instruction, playbook_key AS playbookKey, trigger_type AS triggerType, cadence_minutes AS cadenceMinutes, event_type AS eventType, trust_level AS trustLevel FROM company_routines WHERE id = ? AND workspace_id = ?").bind(body.id, workspaceId).first<Parameters<typeof runCompanyRoutine>[1]>();
      if (!routine) return Response.json({ error: "Routine was not found." }, { status: 404 });
      await runCompanyRoutine(db, routine, "schedule", new Date());
      return Response.json(await workspacePayload(workspaceId, user.id));
    }
    if (body.action === "update_trust_policy") {
      const actionType = body.actionType?.trim().slice(0, 72);
      if (!actionType) return Response.json({ error: "Action type is required." }, { status: 400 });
      const trustLevel = normalizeTrustLevel(body.trustLevel);
      const maxRiskLevel = Math.max(0, Math.min(3, Number(body.maxRiskLevel ?? 1)));
      await db.prepare("INSERT INTO action_trust_policies (workspace_id, action_type, trust_level, max_risk_level, reason, created_at, updated_at) VALUES (?, ?, ?, ?, 'Updated by Founder', ?, ?) ON CONFLICT(workspace_id, action_type) DO UPDATE SET trust_level = excluded.trust_level, max_risk_level = excluded.max_risk_level, reason = excluded.reason, updated_at = excluded.updated_at").bind(workspaceId, actionType, trustLevel, maxRiskLevel, nowText(), nowText()).run();
      return Response.json(await workspacePayload(workspaceId, user.id));
    }
    if (body.action === "dismiss_behavior_rule") {
      if (!body.id) return Response.json({ error: "Rule is required." }, { status: 400 });
      await db.prepare("UPDATE company_behavior_rules SET status = 'inactive', updated_at = ? WHERE id = ? AND workspace_id = ?").bind(nowText(), body.id, workspaceId).run();
      return Response.json(await workspacePayload(workspaceId, user.id));
    }
    ...
    if (body.action === "approve" || body.action === "reject" || body.action === "defer") {
      ...
      const approval = await db.prepare("SELECT task_id AS taskId, action_type AS actionType, title, payload FROM approvals WHERE id = ? AND workspace_id = ?").bind(body.id, workspaceId).first<{ taskId: number; actionType: string; title: string; payload: string }>();
      const feedback = db.prepare("INSERT INTO founder_feedback (workspace_id, target_type, target_id, feedback_type, reason, original_json, created_by) VALUES (?, 'approval', ?, ?, ?, ?, ?) RETURNING id").bind(workspaceId, String(body.id), body.action, body.reason?.trim().slice(0, 1000) || null, approval?.payload ?? "{}", user.email);
      const savedFeedback = await feedback.first<{ id: number }>();
      await db.batch([db.prepare("UPDATE approvals SET status = ?, approved_by = ?, approved_at = ? WHERE id = ? AND workspace_id = ?").bind(status, user.email, nowText(), body.id, workspaceId), db.prepare("UPDATE campaign_assets SET status = ?, updated_at = ? WHERE approval_id = ? AND workspace_id = ?").bind(status === "approved" ? "approved" : status, nowText(), body.id, workspaceId), db.prepare("UPDATE campaigns SET status = ?, updated_at = ? WHERE id = (SELECT campaign_id FROM campaign_assets WHERE approval_id = ? AND workspace_id = ?) AND workspace_id = ?").bind(status === "approved" ? "approved" : status, nowText(), body.id, workspaceId, workspaceId), ...(approval ? [db.prepare("UPDATE agent_tasks SET status = ? WHERE id = ? AND workspace_id = ?").bind(status === "approved" ? "approved" : status, approval.taskId, workspaceId)] : []), ...(body.saveAsRule && body.reason?.trim() && savedFeedback ? [db.prepare("INSERT INTO company_behavior_rules (workspace_id, category, rule_text, source_feedback_id, confidence, status) VALUES (?, ?, ?, ?, 90, 'active')").bind(workspaceId, approval?.actionType ?? "approval", `Avoid ${approval?.actionType ?? "this action"} when: ${body.reason.trim().slice(0, 800)}`, savedFeedback.id)] : [])]);
      if (status === "approved") {
