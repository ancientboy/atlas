...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "publish_campaign_asset") {
      ...
      const asset = await db.prepare("SELECT id, channel, title, content, cta, status FROM campaign_assets WHERE id = ? AND workspace_id = ?").bind(body.id, workspaceId).first<{ id: number; channel: CampaignChannel; title: string; content: string; cta: string; status: string }>();
      if (!asset || asset.status !== "approved") return Response.json({ error: "Approve this campaign asset before publishing." }, { status: 400 });
      if (!isPublishableChannel(asset.channel)) return Response.json({ error: "This channel requires manual publishing." }, { status: 400 });
      const key = `${workspaceId}:${asset.id}:${asset.channel}`;
      await db.prepare("INSERT OR IGNORE INTO publication_jobs (workspace_id, asset_id, idempotency_key, status, created_at, updated_at) VALUES (?, ?, ?, 'queued', ?, ?)").bind(workspaceId, asset.id, key, nowText(), nowText()).run();
      const job = await db.prepare("SELECT id, status, published_url AS publishedUrl, attempt_count AS attemptCount, max_attempts AS maxAttempts FROM publication_jobs WHERE idempotency_key = ? AND workspace_id = ?").bind(key, workspaceId).first<{ id: number; status: string; publishedUrl: string | null; attemptCount: number; maxAttempts: number }>();
      if (!job) return Response.json({ error: "Publication job could not be created." }, { status: 500 });
      if (job.status === "published" && job.publishedUrl) return Response.json(await workspacePayload(workspaceId, user.id));
      ...
      try {
        ...
        await db.batch([
          ...
          db.prepare("UPDATE campaign_assets SET status = 'published', published_url = ?, published_at = ?, updated_at = ? WHERE id = ? AND workspace_id = ?").bind(publishedUrl, nowText(), nowText(), asset.id, workspaceId),
          db.prepare("UPDATE campaigns SET status = 'active', updated_at = ? WHERE id = (SELECT campaign_id FROM campaign_assets WHERE id = ? AND workspace_id = ?) AND workspace_id = ?").bind(nowText(), asset.id, workspaceId, workspaceId),
        ]);
        return Response.json(await workspacePayload(workspaceId, user.id));
      } catch (error) {
        const message = safeClientError(error);
        const retryAt = new Date(Date.now() + Math.min(60, 2 ** (job.attemptCount + 1)) * 60_000).toISOString();
        await db.prepare("UPDATE publication_jobs SET status = CASE WHEN attempt_count >= max_attempts THEN 'failed' ELSE 'retrying' END, next_attempt_at = ?, last_error = ?, updated_at = ? WHERE id = ? AND workspace_id = ?").bind(retryAt, message, nowText(), job.id, workspaceId).run();
