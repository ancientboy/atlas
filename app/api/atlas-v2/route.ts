...
async function snapshot(workspaceId: string) {
  const db = env.DB;
  const [workspace, product, agents, tasks, approvals, runs, opportunities, memories, connections, metrics, campaigns, campaignAssets, growthSnapshots, runtimeSettings, runtimeCycle, runtimeUsage, goals, intelligence] = await Promise.all([
