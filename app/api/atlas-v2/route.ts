...
async function snapshot(workspaceId: string) { const db = env.DB; const [workspace, product, agents, tasks, approvals, runs, opportunities, memories, connections, metrics, campaigns, campaignAssets, growthSnapshots] = await Promise.all([
...
export async function POST(request: Request) {
  try {
    const { user, workspaceId } = await getWorkspace(request);
    const body = await request.json() as { action?: string; id?: number; enabled?: boolean; opportunityId?: number; objective?: string; channels?: string[]; locale?: "zh" | "en"; title?: string; content?: string; cta?: string; publishedUrl?: string; metrics?: { impressions?: number; clicks?: number; conversions?: number }; productName?: string; product?: { name?: string; url?: string; description?: string; growthGoal?: string; locale?: "zh" | "en" } };
    const db = env.DB;
