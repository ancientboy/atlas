...
export async function GET(request: Request) {
  ...
  try {
    ...
    const scopes = provider === "google_search_console" ? '["webmasters.readonly"]' : '[]';
    const label = provider === "google_search_console" ? `Google Search Console · ${identity.label}` : identity.label;
    const metadata = provider === "google_search_console" ? JSON.stringify({ siteUrl: null, lastStatus: "property_required" }) : "{}";
    await env.DB.prepare("INSERT INTO platform_connections (workspace_id, provider, external_account_id, account_label, status, scopes_json, credential_reference, credential_ciphertext, metadata_json, expires_at, last_sync_at, created_at, updated_at) VALUES (?, ?, ?, ?, 'connected', ?, 'workspace_vault', ?, ?, ?, ?, ?, ?) ON CONFLICT(workspace_id, provider, external_account_id) DO UPDATE SET account_label = excluded.account_label, status = 'connected', credential_ciphertext = excluded.credential_ciphertext, metadata_json = CASE WHEN excluded.provider = 'google_search_console' THEN excluded.metadata_json ELSE platform_connections.metadata_json END, expires_at = excluded.expires_at, updated_at = excluded.updated_at").bind(row.workspaceId, provider, identity.id, label, scopes, credentialCiphertext, metadata, token.expiresAt ?? null, new Date().toISOString(), new Date().toISOString(), new Date().toISOString()).run();
