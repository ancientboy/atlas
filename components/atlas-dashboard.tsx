...
function Connections({ t, data, onReflect }: { t: typeof copy.zh; data: AtlasV2Data; onReflect: () => Promise<void> }) {
  ...
  const providers = [{ id: "wordpress", name: "WordPress / CMS", ready: data.publishing?.wordpress, note: zh ? "官方 REST API；默认先保存草稿" : "Official REST API; draft-first by default" }, { id: "x", name: "X", ready: data.publishing?.x, note: zh ? "官方 API 发布与回执" : "Official API publishing and receipt" }, { id: "linkedin", name: "LinkedIn", ready: data.publishing?.linkedin, note: zh ? "官方组织或成员发布接口" : "Official organization or member posting" }, { id: "reddit", name: "Reddit", ready: data.publishing?.reddit, note: zh ? "严格审批；禁止批量回复" : "Strict approval; no bulk replies" }, { id: "analytics", name: "GA4 / PostHog / Atlas", ready: data.publishing?.analytics, note: zh ? "UTM、站内事件与转化归因" : "UTM, first-party events, and attribution" }, { id: "xiaohongshu", name: "小红书", ready: false, note: zh ? "人工发布与链接回填" : "Manual publishing with URL receipt" }];
  return <><section className="view-title"><p>OBSERVE · PUBLISH · MEASURE</p><h1>{t.connections}</h1><span>{t.connectionLead}</span><button className="review" onClick={() => void onReflect()}>{zh ? "立即生成今日复盘" : "Run today's reflection"}</button></section><section className="connection-blueprint"><header><div><p>AUTOMATED PUBLISHING</p><h2>{zh ? "绑定一次，Atlas 才能安全执行和每日复盘" : "Connect once so Atlas can safely publish and review daily"}</h2></div><span>{zh ? "官方授权 · 最小权限 · 随时撤销" : "Official auth · least privilege · revocable"}</span></header><div>{[{ n: "01", z: "绑定发布账号", e: "Connect publisher", d: zh ? "X、LinkedIn、Reddit 或 CMS 只使用官方接口，不提交平台密码。" : "X, LinkedIn, Reddit, and CMS connections use official APIs—never platform passwords." }, { n: "02", z: "批准或设定自动规则", e: "Approve or automate", d: zh ? "所有新渠道默认逐条审批；之后才能限定渠道和风险等级自动执行。" : "New channels default to per-item approval before scoped automation is allowed." }, { n: "03", z: "自动发布并取得回执", e: "Publish with receipt", d: zh ? "Atlas 使用幂等任务、防重复发布、失败重试并保存公开链接。" : "Atlas uses idempotent jobs, duplicate protection, retries, and public receipts." }, { n: "04", z: "每日同步增长效果", e: "Daily growth sync", d: zh ? "平台曝光与点击结合 UTM、站内事件和转化回传完成归因。" : "Platform reach and clicks combine with UTM, first-party events, and conversion events." }].map((item) => <article key={item.n}><b>{item.n}</b><strong>{zh ? item.z : item.e}</strong><span>{item.d}</span></article>)}</div></section><div className="connection-grid provider-grid">{providers.map((item) => <article key={item.id}><div><span>{item.id === "analytics" ? (zh ? "数据" : "Analytics") : (zh ? "分发" : "Distribution")}</span><b className={item.ready ? "connected" : "available"}>{item.ready ? t.connected : item.id === "xiaohongshu" ? (zh ? "仅手动" : "Manual only") : (zh ? "待配置" : "Needs configuration")}</b></div><h2>{item.name}</h2><p>{item.note}</p><footer>{item.ready ? (zh ? "服务端凭据已就绪" : "Server-side credentials ready") : (zh ? "不会自动发布" : "No automatic publishing")}</footer></article>)}</div></>;
}
