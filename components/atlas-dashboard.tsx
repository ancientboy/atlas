...
function ContentStudioAsset({ locale, productName, productUrl, asset, onAssetAction, onOpenApprovals }: { locale: Locale; productName: string; productUrl: string; asset: CampaignAsset; onAssetAction: (action: string, asset: CampaignAsset, payload?: Record<string, unknown>) => Promise<void>; onOpenApprovals: () => void }) {
  const zh = locale === "zh";
  ...
  return <article className={`content-studio-asset channel-${asset.channel}`}>
    ...
    {asset.status === "approved" && <div className="publish-ready"><span>✓</span><div><strong>{zh ? "内容已批准，可以发布" : "Approved and publish ready"}</strong><small>{zh ? "Atlas 目前不会自动发布；请复制、导出或发布后记录链接。" : "Atlas does not auto-publish yet. Copy, export, or record the live URL after publishing."}</small></div></div>}
...
function Connections({ t, data }: { t: typeof copy.zh; data: AtlasV2Data }) {
  const zh = t.workspace === copy.zh.workspace;
  return <><section className="view-title"><p>OBSERVE · PUBLISH · MEASURE</p><h1>{t.connections}</h1><span>{t.connectionLead}</span></section><section className="connection-blueprint"><header><div><p>AUTOMATED PUBLISHING</p><h2>{zh ? "绑定一次，Atlas 才能安全执行和每日复盘" : "Connect once so Atlas can safely publish and review daily"}</h2></div><span>{zh ? "OAuth 授权 · 最小权限 · 随时撤销" : "OAuth · least privilege · revocable"}</span></header><div>{[{ n: "01", z: "绑定发布账号", e: "Connect publisher", d: zh ? "X、LinkedIn 或 CMS 通过官方授权连接，不提交平台密码。" : "Connect X, LinkedIn, or a CMS through official authorization—never platform passwords." }, { n: "02", z: "批准或设定自动规则", e: "Approve or automate", d: zh ? "可以逐条审批，也可以只对指定渠道、时间和风险等级自动发布。" : "Approve each asset or allow publishing only for selected channels, times, and risk levels." }, { n: "03", z: "自动发布并取得回执", e: "Publish with receipt", d: zh ? "Atlas 保存平台帖子 ID、公开链接和失败重试记录。" : "Atlas stores the platform post ID, public URL, and retry history." }, { n: "04", z: "每日同步增长效果", e: "Daily growth sync", d: zh ? "平台曝光与点击结合 UTM、分析工具和转化回传完成归因。" : "Platform reach and clicks combine with UTM, analytics, and conversion events for attribution." }].map((item) => <article key={item.n}><b>{item.n}</b><strong>{zh ? item.z : item.e}</strong><span>{item.d}</span></article>)}</div></section><div className="connection-grid">{data.connections.map((item) => <article key={item.id}><div><span>{item.category}</span><b className={item.status}>{item.status === "connected" ? t.connected : item.status === "mock" ? t.mock : t.available}</b></div><h2>{item.name}</h2><p>{item.description}</p><footer>{t.lastSync}: {item.lastSync}</footer></article>)}</div></>;
}
