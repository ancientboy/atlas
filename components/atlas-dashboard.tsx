...
function Campaigns({ locale, data, onAssetAction, onOpenApprovals }: { locale: Locale; data: AtlasV2Data; onAssetAction: (action: string, asset: CampaignAsset, payload?: Record<string, unknown>) => void; onOpenApprovals: () => void }) {
  const zh = locale === "zh";
  const campaigns = data.campaigns ?? [];
  const assets = data.campaignAssets ?? [];
  const copyAsset = async (asset: CampaignAsset) => { await navigator.clipboard.writeText(`${asset.title}\n\n${asset.content}\n\n${asset.cta}`); };
  return <><section className="view-title campaign-view-title"><p>GROWTH CAMPAIGNS</p><h1>{zh ? "增长活动" : "Campaigns"}</h1><span>{zh ? "从机会到内容、审批、发布和效果反馈的执行闭环。" : "The execution loop from opportunity to content, approval, publishing, and results."}</span></section>{campaigns.length === 0 ? <div className="empty-v2 campaign-empty"><strong>{zh ? "还没有 Campaign" : "No campaigns yet"}</strong><span>{zh ? "前往增长机会，选择一个机会创建 Campaign。" : "Open Opportunities and turn one into a campaign."}</span></div> : <div className="campaign-list">{campaigns.map((campaign) => <section key={campaign.id} className="campaign-card"><header><div><span>{campaign.status}</span><h2>{campaign.name}</h2><p>{campaign.objective}</p></div><dl><div><dt>{zh ? "受众" : "Audience"}</dt><dd>{campaign.audience}</dd></div><div><dt>{zh ? "核心信息" : "Core message"}</dt><dd>{campaign.coreMessage}</dd></div><div><dt>CTA</dt><dd>{campaign.cta}</dd></div></dl></header><div className="campaign-assets">{assets.filter((asset) => asset.campaignId === campaign.id).map((asset) => <article key={asset.id}><div className="campaign-asset-head"><span>{asset.channel}</span><b className={`state ${statusTone(asset.status)}`}>{asset.status}</b></div><h3>{asset.title}</h3><pre>{asset.content}</pre><strong>CTA · {asset.cta}</strong><div className="campaign-asset-actions"><button onClick={() => void copyAsset(asset)}>{zh ? "复制内容" : "Copy"}</button>{asset.status === "pending_approval" && <button onClick={onOpenApprovals}>{zh ? "前往审批" : "Review approval"}</button>}{asset.status === "approved" && <button className="publish" onClick={() => { const url = window.prompt(zh ? "可选：粘贴发布后的链接" : "Optional: paste the published URL", "") ?? ""; onAssetAction("mark_campaign_asset_published", asset, { publishedUrl: url }); }}>{zh ? "标记已发布" : "Mark published"}</button>}</div>{asset.status === "published" && <CampaignMetrics locale={locale} asset={asset} onSave={(metrics) => onAssetAction("update_campaign_metrics", asset, { metrics })} />}</article>)}</div></section>)}</div>}</>;
}
