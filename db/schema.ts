...
export const growthExperiments = sqliteTable("growth_experiments", { id: integer("id").primaryKey({ autoIncrement: true }), workspaceId: text("workspace_id").notNull(), planId: integer("plan_id"), decisionId: integer("decision_id"), name: text("name").notNull(), hypothesis: text("hypothesis").notNull(), strategyKey: text("strategy_key").notNull(), channel: text("channel"), primaryMetric: text("primary_metric").notNull(), baselineValue: real("baseline_value"), targetValue: real("target_value"), evaluationStartsAt: text("evaluation_starts_at"), evaluationEndsAt: text("evaluation_ends_at"), status: text("status").notNull().default("planned"), ...timestamps });
