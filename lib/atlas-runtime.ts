...
export function safeClientError(error: unknown) { const message = error instanceof Error ? error.message : "Request failed."; if (/LLM key|http\/https|credentials|ports 80 and 443|Private|metadata|HTML|exceeds|already running|rate limit|Invalid analysis|Trusted DNS resolver/.test(message)) return message; return "Analysis failed. Please retry later."; }
