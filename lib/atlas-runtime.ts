...
export function safeClientError(error: unknown) { const message = error instanceof Error ? error.message : "Request failed."; if (/Missing server LLM key|server LLM API key|Custom LLM provider|LLM_BASE_URL|LLM provider host|http\/https|credentials|ports 80 and 443|Private|metadata|HTML|exceeds|already running|rate limit|Invalid analysis|LLM request timed out|LLM response body exceeds|Trusted DNS resolver|Hostname/.test(message)) return message; return "Analysis failed. Please retry later."; }
