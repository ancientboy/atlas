...
export type CompanyRuntimeSettings = { enabled: number; mode: "manual" | "copilot" | "autonomous" | "paused"; tickIntervalMinutes: number; dailyActionLimit: number; dailyLlmBudgetCents: number; dailyExternalActionLimit: number; quietHoursStart: string | null; quietHoursEnd: string | null; timezone: string; autoExecuteRiskLevel: number; pausedReason: string | null; lastTickAt: string | null; nextTickAt: string | null };
export type CompanyRuntimeCycle = { id: number; status: string; triggerType: string; currentStage: string; startedAt: string; completedAt: string | null; observationsCount: number; opportunitiesCount: number; plansCount: number; tasksCreatedCount: number; tasksExecutedCount: number; approvalsCreatedCount: number; summary: string | null; errorMessage: string | null };
export type CompanyIntelligenceSnapshot = { snapshotDate: string; goalId: number | null; metricName: string | null; metricValue: number | null; previousValue: number | null; metricDelta: number | null; healthScore: number; evidence: string[]; summary: string; updatedAt: string };
export type DecisionBreakdown = { evidence: number; goalAlignment: number; knowledgeFit: number; historicalPerformance: number; costEfficiency: number; riskFit: number; channelCapacity: number };
export type CompanyBrain = { facts: { factType: string; subject: string; value: unknown; source: string; confidence: number; status: string; lastVerifiedAt: string }[]; knowledgePacks: { packKey: string; name: string; description: string; industry: string | null; stage: string | null; channel: string | null; version: number }[]; decisions: { id: number; title: string; rationale: string; score: number; confidence: number; status: string; createdAt: string; alternatives: { opportunityId: number; title: string; score: number; strategyKey: string; channel: string; breakdown: DecisionBreakdown; reason: string }[]; context: { version?: number; strategyKey?: string; channel?: string; riskLevel?: number; estimatedCost?: string; breakdown?: DecisionBreakdown } }[]; experiments: { id: number; name: string; hypothesis: string; strategyKey: string; primaryMetric: string; status: string; evaluationEndsAt: string | null }[]; lessons: { strategyKey: string; lessonType: string; statement: string; confidence: number; status: string }[] };
export type AtlasV2Data = typeof atlasV2Seed & { product?: Product | null; workspace?: { id: string; autonomyEnabled?: boolean; autonomyUpdatedAt?: string | null }; workspaces?: WorkspaceSummary[]; campaigns?: Campaign[]; campaignAssets?: CampaignAsset[]; growthSnapshots?: DailyGrowthSnapshot[]; platformConnections?: PlatformConnection[]; runtime?: { schedule: RuntimeSchedule | null; jobs: RuntimeJob[] }; companyRuntime?: { settings: CompanyRuntimeSettings | null; latestCycle: CompanyRuntimeCycle | null; usage: { cyclesCount: number; actionsCount: number; externalActionsCount: number; estimatedCostCents: number }; goals: { id: number; title: string; targetMetric: string | null; targetValue: number | null; currentValue: number | null; status: string }[] }; companyIntelligence?: CompanyIntelligenceSnapshot | null; companyBrain?: CompanyBrain; observationEngine?: { sources: ObservationSourceState[]; insights: Insight[] }; oauthApps?: { x: boolean; linkedin: boolean; reddit: boolean; google_search_console: boolean }; publishing?: { wordpress: boolean; x: boolean; linkedin: boolean; reddit: boolean; analytics: boolean }; metrics: { visits: number; signups: number; paid: number; conversion: number } };
