...
export async function runCompanyRuntimeCycle(db: Db, workspaceId: string, trigger: RuntimeTrigger, options: { now?: Date; idempotencyKey?: string; env?: Record<string, string | undefined> } = {}) {
  ...
  try {
    ...
    cycleId = created.id;
    await recordRuntimeHeartbeat(db, { workspaceId, source: trigger, status: "running", startedAt, details: { cycleId } });
    ...
    await ensureCompanyBrain(db, workspaceId, now);
    const intelligence = await refreshCompanyIntelligence(db, workspaceId, now);
    const experimentLoop = await advanceWorkspaceExperiments(db, workspaceId, now);
    const companyFunctions = await refreshCompanyFunctions(db, workspaceId, now);
    ...
    if (opportunity && state.agentId) {
      ...
      taskId = task?.id ?? null;
      if (taskId && policy.decision === "require_approval") { await db.prepare("INSERT OR IGNORE INTO approvals (workspace_id, task_id, action_type, title, reason, payload, risk_level, status, created_at) VALUES (?, ?, 'company_runtime_plan', ?, ?, ?, ?, 'pending', ?)").bind(workspaceId, taskId, `Approve plan: ${opportunity.title}`.slice(0, 220), policy.reason, JSON.stringify({ planId, opportunityId: opportunity.id }), selection.decision.riskLevel, startedAt).run(); approvalCount += 1; }
      const execution = await db.prepare("INSERT OR IGNORE INTO action_executions (workspace_id, cycle_id, plan_id, task_id, agent_id, action_type, risk_level, policy_decision, status, input_json, output_json, idempotency_key, started_at, completed_at) VALUES (?, ?, ?, ?, ?, 'create_company_action', ?, ?, ?, ?, ?, ?, ?, ?)").bind(workspaceId, cycleId, planId, taskId, state.agentId, selection.decision.riskLevel, policy.decision, policy.decision === "execute" ? "completed" : policy.decision === "require_approval" ? "waiting_approval" : "blocked", JSON.stringify({ opportunityId: opportunity.id, strategyKey: selection.decision.strategyKey }), JSON.stringify({ policy: policy.policyCode, taskId }), `runtime-action:${cycleId}:create_company_action`, startedAt, policy.decision === "execute" ? startedAt : null).run();
    ...
    await recordRuntimeHeartbeat(db, { workspaceId, source: trigger, status: "healthy", startedAt, completedAt: nowText(), nextExpectedAt: new Date(now.getTime() + settings.tickIntervalMinutes * 60_000).toISOString(), details: { cycleId, experiments: experimentLoop.considered, functions: companyFunctions.length } });
    return { skipped: false, cycleId, summary, planId, taskId, autonomy, experimentLoop, companyFunctions };
  } catch (error) {
    ...
    await recordRuntimeHeartbeat(db, { workspaceId, source: trigger, status: "failed", startedAt, completedAt: nowText(), details: { cycleId, error: error instanceof Error ? error.message.slice(0, 200) : "Runtime cycle failed" } });
    throw error;
